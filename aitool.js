// This file assumes 'cateTool' is available globally from 'dynamicarea.js'
// and 'aitool' is the container div in your HTML.

const dynamicContentArea = document.getElementById('aitool');
let allTools = [];
let currentFocus = -1;
...
function renderDynamicContent() {
    ...
    if (typeof cateTool === 'undefined' || !Array.isArray(cateTool)) {
        console.error("cateTool is not defined or is not an array. Ensure dynamicarea.js is loaded correctly.");
        return;
    }

    cateTool.forEach(category => {
        // Create <details> wrapper
        const details = document.createElement('details');
        details.open = false; // start collapsed, set to true if you want default expanded

        // Create <summary> to replace category title
        const summary = document.createElement('summary');
        summary.classList.add('category-title');
        ...
        // --- START OF NEW CODE TO ADD THE COLLAPSE BUTTON ---
        const collapseButton = document.createElement('button');
        collapseButton.classList.add('collapse-category-button');
        collapseButton.textContent = '▲';
        
        collapseButton.addEventListener('click', () => {
            // Find the parent <details> element (which is 'details' itself here)
            // and set its 'open' attribute to false to collapse it.
            details.open = false;

            // Optional: Scroll to the category title after collapsing
            const categoryTitle = details.querySelector('summary.category-title');
            if (categoryTitle) {
        ...
        // Append everything to dynamic content area
        dynamicContentArea.appendChild(details);
    });
}

// ... rest of your existing JavaScript code ...
// The DOMContentLoaded listener should call renderDynamicContent as it already does.
// The previous event listener for .collapse-category-button can be removed
// or kept (it won't hurt, but won't find anything as the event listener is now
// added directly during creation).

// You can remove the separate DOMContentLoaded listener for collapse buttons
// that I provided in the previous answer, as the event listener is now
// attached directly when the button is created.
...
function highlightAndScroll(cardElement) {
    ...
    if (cardElement) {
        // ✅ Expand the parent <details> element if collapsed
        const detailsParent = cardElement.closest('details');
...
document.addEventListener('DOMContentLoaded', () => {
    renderDynamicContent(); // This will now create the buttons and attach listeners

    const searchInput = document.getElementById('input-box');
    const suggestionBox = document.getElementById('suggestion-box');
    // Re-select toolCards after rendering content
    const toolCards = document.querySelectorAll('.tool-card'); 
    ...
    // Populate allTools AFTER renderDynamicContent has run
